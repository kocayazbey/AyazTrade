...
export default function PendingOrdersPage() {
  ...
  const fetchPendingOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/proxy/orders?status=pending', {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        ...
        throw new Error(data.message || 'Bekleyen siparişler yüklenemedi');
      }
    } catch (error) {
      console.error('Error fetching pending orders:', error);
      toast.error('Bekleyen siparişler yüklenirken hata oluştu');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Bekleyen Siparişler</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            {filteredOrders.length} bekleyen sipariş
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        {!loading && filteredOrders.length === 0 && (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">Bekleyen sipariş bulunamadı</p>
          </div>
        )}
      </div>
    </div>
