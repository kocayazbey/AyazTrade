...
export class IPFilterService {
  ...
  async isBanned(ip: string): Promise<{
    banned: boolean;
    reason?: string;
    ...
    const tempBlock = await this.redis.get(`ip:tempblock:${ip}`);
    if (tempBlock) {
      const ttl = await this.redis.ttl(`ip:tempblock:${ip}`);
      return {
        banned: true,
        reason: 'IP temporarily blocked due to suspicious activity',
  ...
  async trackSuspiciousActivity(ip: string): Promise<void> {
    const threshold = this.configService.get<number>('IP_SUSPICIOUS_ACTIVITY_THRESHOLD', 10);
    const windowHours = this.configService.get<number>('IP_SUSPICIOUS_ACTIVITY_WINDOW_HOURS', 1);
    ...
    // Auto-block after configured threshold of suspicious activities
    if (count >= threshold) {
      const banDuration = this.configService.get<number>('IP_BAN_DURATION_MINUTES', 60) * 60;
      await this.tempBlock(ip, banDuration, `Multiple suspicious activities detected (${count} attempts)`);
    }
  }

  async trackFailedLoginAttempt(ip: string): Promise<void> {
    const threshold = this.configService.get<number>('IP_BAN_FAILED_ATTEMPTS', 5);
    const windowMinutes = this.configService.get<number>('IP_BAN_WINDOW_MINUTES', 15);
    const windowSeconds = windowMinutes * 60;

    const key = `ip:failed_login:${ip}`;
    const count = await this.redis.incr(key);
    await this.redis.expire(key, windowSeconds);

    this.logger.warn(`Failed login attempt from IP: ${ip} (attempt ${count}/${threshold})`);

    // Auto-block after configured threshold of failed login attempts
    if (count >= threshold) {
      const banDuration = this.configService.get<number>('IP_BAN_DURATION_MINUTES', 60) * 60;
      await this.tempBlock(ip, banDuration, `Multiple failed login attempts (${count} attempts in ${windowMinutes} minutes)`);

      // Also track as general suspicious activity
      await this.trackSuspiciousActivity(ip);
    }
  }
