...
export class KPIService {
  ...
  async getKPIs(tenantId: string, category?: string) {
    if (category) {
      return await this.db.select().from(kpis).where(
        and(eq(kpis.tenantId, tenantId), eq(kpis.category, category))
      );
    }

    return await this.db.select().from(kpis).where(eq(kpis.tenantId, tenantId));
  }
    ...
    const periodKPIs = await this.db
      ...
      .where(
        and(
          ...
          between(kpis.periodStart, periodStart.toISOString().split('T')[0], periodEnd.toISOString().split('T')[0]) as any
        )
      );
